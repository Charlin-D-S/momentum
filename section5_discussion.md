...
La section précédente a établi les trois grilles et leur validation de base. Cette discussion en éprouve la valeur. Elle répond à six questions : les grilles sont-elles performantes et proches du plafond atteignable, sont-elles homogènes et bien calibrées, battent-elles le score existant, faut-il vraiment trois grilles, un modèle général suffit-il sur le périmètre standard, et le modèle est-il représentatif de la population d'octroi. Le Pros-ER sert de fil rouge ; les résultats complets des Associations et des SCI figurent en annexe. Les emplacements `[DONNÉE]` reçoivent les chiffres issus des données de la banque.

---

## 5.1 Performance des trois grilles et proximité de la borne
...
L'écart entre la grille et le challenger mesure le prix de l'interprétabilité. Sa faiblesse rejoint un fait ancien du scoring de crédit : sur ce type de données, les méthodes de classification se tiennent dans un intervalle étroit, le *flat maximum* décrit par Hand et Henley (1997). Renoncer à la boîte noire coûte donc peu, ici `[DONNÉE]` points de Gini, au regard du gain de transparence.
...
Sur le Pros-ER, la nouvelle grille porte le Gini de `[DONNÉE]`, celui du score existant, à `[DONNÉE]`, soit un gain de `[DONNÉE]` points. Les courbes ROC superposées montrent une dominance `[DONNÉE]` de la nouvelle grille (Figure 5.3), et les gains se lisent tranche de score par tranche de score (Table 5.4).
...
**Figure 5.3** — Courbes ROC de la nouvelle grille et du score existant, Pros-ER `[DONNÉE]`.
**Table 5.4** — Gains par tranche de score, nouvelle grille contre score existant `[DONNÉE]`.

---

## 5.4 Faut-il trois grilles ou une seule ?
...
Pour y répondre, la grille du Pros-ER est appliquée aux Associations et aux SCI, et sa performance comparée à celle des grilles dédiées (Table 5.5). L'écart ressort à `[DONNÉE]` points de Gini sur les Associations et `[DONNÉE]` sur les SCI.
...
**Table 5.5** — Performance de la grille standard sur chaque périmètre, comparée à la grille dédiée `[DONNÉE]`.

---

## 5.5 Un modèle général suffit-il sur le Pros-ER ? Le test A/B des libellés
...
> **[Contribution]** Pour isoler l'apport des libellés, la comparaison est menée à population constante. Sur les seuls clients à au moins 5 transactions, on oppose le modèle général sans libellés au modèle spécialisé avec libellés (Figure 5.4). La population étant identique, la différence de performance ne peut venir que des libellés.

Le gain attribuable aux libellés ressort à `[DONNÉE]` points de Gini, à situer par rapport à la borne du challenger (Table 5.6). Ce gain `[DONNÉE : dépasse / ne dépasse pas]` le coût de maintenir deux modèles et un pipeline de libellés.
...
**Figure 5.4** — Design du test à population constante, général sans libellés contre spécialisé avec libellés `[DONNÉE]`.
**Table 5.6** — Performance des options et gain attribuable aux libellés, situé sous la borne du challenger `[DONNÉE]`.

---

## 5.6 Le modèle est-il représentatif de la population d'octroi ?
...
## 5.7 Robustesse des choix méthodologiques
...
L'estimation sur variables indicatrices est comparée à un encodage des classes en poids de preuve suivi d'une régression, à la manière de Siddiqi (2006). Les deux approches convergent `[DONNÉE]`, ce qui montre que le choix du codage n'affecte pas les conclusions.
...
Les limites doivent être posées avec la même clarté. Le modèle se tient hors du cadre IRB homologué, dans un usage opérationnel. La calibration est laissée secondaire, ce qui suffit tant que le seuil vient du métier, mais limiterait un usage probabiliste direct. L'hypothèse d'indépendance des observations reste une approximation, dont la section précédente a mesuré la portée. Le périmètre couvert, enfin, borne la généralité des conclusions.
